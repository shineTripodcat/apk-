...
### 快速开始
```bash
右上角Fork克隆本项目，去actions手动触发自动编译流程，2分钟后就能获取最新ipk压缩包`luci-app-lucky.zip`解压上传到Openwrt软路由安装即可
...
opkg install /tmp/tmp/luci-app-lucky_all.ipk

#卸载
opkg remove luci-app-lucky

#更新版本需要先卸载再安装新的ipk然后去管理界面关闭插件 修改参数后重新点击应用并保存
#安装后openwrt管理界面里不显示lucky 请注销登录或关闭窗口重新打开  
```

```bash
#如果是新版openwrt使用的是apk包管理器 出现无法安装apk的可以尝试使用忽略证书验证
apk add --allow-untrusted /tmp/tmp/luci-app-lucky.apk
```

此luci-app-lucky不包含二进制程序，需要自行在openwrt管理界面里的lucky插件界面里上传二进制程序
...
#克隆luci-app-lucky到sdk的package目录里
git clone https://github.com/lucky/luci-app-lucky.git /opt/luci-app-lucky
cp -R /opt/luci-app-lucky/luci-app-lucky .
...
```
sed -i 's/util/xml/g' /usr/lib/lua/luci/model/cbi/lucky.lua
```
