...
local ipv6_enable = s:option(Flag, "ipv6_enabled", translate("Enable IPv6 Support"), translate("Apply speed limits to IPv6 addresses using MAC address identification"))
ipv6_enable.rmempty = false
ipv6_enable:depends("enabled", "1")
...
-- Also show IPv6 neighbors for reference
ipc.neighbors({family = 6, dev = "br-lan"}, function(n)
	if n.mac and n.dest and not n.dest:string():match("^fe80") then
		ip:value(n.dest:string(), "%s (%s) [IPv6]" %{ n.dest:string(), n.mac })
	end
end)
